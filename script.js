...
class TaxFilingGame {
    ...
    generateFormTemplates() {
        console.log('🔥 GENERATING SIMPLIFIED FORMS - 5 FIELDS ONLY!');
        const taxForms = {
        ...
        // Debug: Log field counts for each form
        console.log('📊 FORM FIELD COUNTS:');
        Object.keys(taxForms).forEach(key => {
            console.log(`${key}: ${taxForms[key].fields.length} fields`);
        });
    ...
    startGame() {
        ...
        console.log('Game started, form state:', this.formState);
    }
    ...
    renderForm() {
        ...
        console.log(`🎯 RENDERING FORM: ${this.currentForm.title}`);
        console.log(`🔢 FIELD COUNT: ${this.currentForm.fields.length} fields`);
        console.log(`📋 FIELDS:`, this.currentForm.fields.map(f => f.label));
        ...
        setTimeout(() => {
            this.currentForm.fields.forEach((field, index) => {
                ...
                if (element) {
                    ...
                    console.log(`Field ${fieldId} disabled: ${element.disabled}`);
                }
            });
        }, 100);
    }
    ...
    fillForm() {
        console.log('fillForm() called, current state:', this.formState);
        
        if (this.formState !== 'empty') {
            console.log('Form state is not empty, returning');
            return;
        }
        
        console.log('Setting form state to filled');
    ...
    showStartScreen() {
        ...
        screens.forEach(screenId => {
            ...
            if (screen) {
                screen.style.display = screenId === 'start-screen' ? 'block' : 'none';
            }
        });
...
function startGame() {
    // Ultra simple version that just calls forceStartGame
    console.log('startGame() called - redirecting to forceStartGame');
    forceStartGame();
}
...
function fillForm() {
    console.log('Global fillForm() called');
    if (game) {
        console.log('Game exists, calling game.fillForm()');
        game.fillForm();
    } else {
...
function createUserProfile() {
    console.log('DIRECT ONCLICK - Create profile clicked');
    const nameInput = document.getElementById('player-name');
    const playerName = nameInput ? nameInput.value.trim() : '';
    
    console.log('Name:', playerName, 'Avatar:', selectedAvatarGlobal);
...
function selectAvatar(avatarType, element) {
    console.log('DIRECT ONCLICK - Avatar selected:', avatarType);
    
    // Update debug display
    const debugAvatar = document.getElementById('debug-avatar');
    const debugStatus = document.getElementById('debug-status');
    if (debugAvatar) debugAvatar.textContent = avatarType;
    if (debugStatus) debugStatus.textContent = 'Avatar selected!';
    ...
    if (createBtn && nameInput && nameInput.value.trim() && selectedAvatarGlobal) {
        ...
        if (debugStatus) debugStatus.textContent = 'Ready to create profile!';
        console.log('Profile is ready!');
    }
}
...
function checkProfileReady() {
    ...
    // Update debug display
    if (debugName && nameInput) {
        debugName.textContent = nameInput.value.trim() || 'Not entered';
    }
    ...
    if (createBtn && nameInput && nameInput.value.trim() && selectedAvatarGlobal) {
        ...
        if (debugStatus) debugStatus.textContent = nameInput?.value.trim() ? 'Need avatar' : 'Need name and avatar';
    }
}
